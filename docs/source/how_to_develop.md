...
mkdir ~/nfs
sudo mount -t nfs -o nolock 192.168.0.100:/home/xxx/workspace/nfs ~/nfs
```
...
在开启了all.launch的情况下，另启一个终端，通过如下命令测试建图和导航效果。
...
```shell
cd newbot_ws/src/config && bash install.sh
```
...
防止风扇频繁启动产生噪音，默认已经将风扇的打开阈值修改为70度，如果需要自定义，通过如下方法修改dtb文件完成：
...
cooling-levels表示pwm占空比，temp-trips表示不同的温度对应的不同档位
...
```shell
	pwm-fan {
		...
		cooling-levels = <0x00 0x01 0x01 0x01 0x01 0xff>;
		rockchip,temp-trips = <0xc350 0x01 0xd6d8 0x02 0xea60 0x03 0xfde8 0x04 0x11170 0x05>;
		status = "okay";
		phandle = <0x30a>;
	};
